  ...
  begin try
      ...
      match e, !opt_file with
        | Syntax.ITGL.Exp _, None -> 
            ...
            close_out oc;
            let _ = Sys.command "gcc result_C/stdout.c lib/cast.c -I/mnt/c/gc/include /mnt/c/gc/lib/libgc.so -o result/stdout.out -O2 -pg" in
        ...
        | Syntax.ITGL.Exp _, Some f -> 
            ...
            Printf.fprintf oc "%s" c_code;
            close_out oc;
            let _ = Sys.command ("gcc ../result_C/"^f^"_out.c ../lib/cast.c -I/mnt/c/gc/include /mnt/c/gc/lib/libgc.so -o ../result/"^f^".out -O2 -pg") in
